...
class Trainer:
    ...
    def run(
        ...
        if trainer_options.resume and (output_dir / "checkpoint.pth").exists():
            ...
            resume_epoch = reporter.get_epoch()
        else:
            resume_epoch = 0

        if sniper_conf:
            device = "cuda" if trainer_options.ngpu > 0 else "cpu",
            sniper_dir = sniper_conf.pop("sniper_dir")
            sniper = SniperTraining(sniper_dir=sniper_dir, device=device, logger=logging.root)
            ...
            optim_lr = sniper_conf.pop("optim_lr")
            sniper.train(
                model=model,
                snip_module_name="tts",
                batch_iterator=batch_iterator,
                get_loss_fn=get_loss_fn,
                train_dtype=trainer_options.train_dtype,
